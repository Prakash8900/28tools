(function(){
  ...
  function loadFiles(fileList){
    ...
    Promise.all(files.map(f=>new Promise(r=>{const i=new Image();i.onload=()=>r(i);i.src=URL.createObjectURL(f);}))).then(imgs=>{
      loadedImgs=imgs;
      document.getElementById('fileListArea').innerHTML=files.map((f,i)=>`<div class="file-item"><span class="file-item__icon">🖼️</span><span class="file-item__name">${f.name}</span><span class="file-item__size">${28tools.formatBytes(f.size)}</span></div>`).join('');
  ...
  processBtn.addEventListener('click',async()=>{
    ...
    script.onload=async()=>{
      ...
      sizeInfo.textContent=`PDF | ${28tools.formatBytes(blob.size)}`;
  ...
  downloadBtn.addEventListener('click',()=>{ if(window._pdfBlob) 28tools.downloadBlob(window._pdfBlob,'images-to-pdf.pdf'); });
