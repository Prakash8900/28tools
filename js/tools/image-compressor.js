...
(function() {
  ...
  function previewFile(file) {
    ...
    img.onload = () => {
      ...
      sizeInfo.textContent = `Original: ${28tools.formatBytes(file.size)} | ${img.naturalWidth}×${img.naturalHeight}`;
    };
    img.src = url;
  }
  ...
  compressBtn.addEventListener('click', async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      showMsg('info', `Compressed ${i+1}/${files.length}: ${28tools.formatBytes(files[i].size)} → ${28tools.formatBytes(blob.size)} (${Math.round((1-blob.size/files[i].size)*100)}% saved)`);
    }
    ...
    img.onload = () => {
      ...
      sizeInfo.textContent = `Compressed: ${28tools.formatBytes(compressedBlobs[0].blob.size)} | ${img.naturalWidth}×${img.naturalHeight}`;
    };
    img.src = firstUrl;
  ...
  downloadBtn.addEventListener('click', async () => {
    if (compressedBlobs.length === 1) {
      28tools.downloadBlob(compressedBlobs[0].blob, compressedBlobs[0].name);
    } else {
      ...
      script.onload = async () => {
        ...
        28tools.downloadBlob(zipBlob, 'compressed-images.zip');
