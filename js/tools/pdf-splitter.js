(function(){
  ...
  function loadPDF(file){
    ...
    s.onload=async()=>{
      ...
      sizeInfo.textContent=`${pdfDoc.getPageCount()} pages | ${ToolsApp.formatBytes(file.size)}`;
  ...
  processBtn.addEventListener('click',async()=>{
    ...
    previewBody.innerHTML=`<div class="status-msg status-msg--success">✅ Split into ${blobs.length} file(s)</div><ul style="margin-top:12px;font-size:.9rem">${blobs.map(b=>`<li style="padding:4px 0;color:var(--clr-text-muted)">📄 ${b.name} (${ToolsApp.formatBytes(b.blob.size)})</li>`).join('')}</ul>`;
  ...
  downloadBtn.addEventListener('click',async()=>{
    ...
    if(blobs.length===1){ToolsApp.downloadBlob(blobs[0].blob,blobs[0].name);return;}
    ...
    s.onload=async()=>{ const zip=new JSZip(); blobs.forEach(({blob,name})=>zip.file(name,blob)); const zb=await zip.generateAsync({type:'blob'}); ToolsApp.downloadBlob(zb,'split-pages.zip'); downloadBtn.disabled=false;downloadBtn.innerHTML='⬇️ Download ZIP'; };
    document.head.appendChild(s);
  });
