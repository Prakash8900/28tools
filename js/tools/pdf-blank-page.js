(function () {
  ...
  async function insertBlanks() {
    ...
    if (!posInput) { ToolsApp.showStatus('statusArea', 'error', 'Enter at least one insertion position.'); return; }
    ...
    if (!positions.length) { ToolsApp.showStatus('statusArea', 'error', 'Invalid positions. Use numbers separated by commas.'); return; }

    for (const p of positions) {
      if (p < 0 || p > pageCount) {
        ToolsApp.showStatus('statusArea', 'error', `Position ${p} is out of range. PDF has ${pageCount} pages (0 to ${pageCount}).`);
        return;
      }
    }

    ToolsApp.showStatus('statusArea', 'info', 'Inserting blank pages…');
    ...
    try {
      ...
      ToolsApp.showStatus('statusArea', 'success', `Done! ${blankCount * positions.length} blank page(s) inserted. New total: ${newPageCount} pages.`);
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBytes) {
      ...
      ToolsApp.downloadBlob(blob, `${base}-with-blanks.pdf`);
    }
  });
  ...
  function loadFile(file) {
    ...
    reader.onload = async e => {
      ...
      resultBytes = null;
      ToolsApp.showStatus('statusArea', 'success', 'PDF loaded. Set positions and click Insert.');
    };
