(function () {
  ...
  function addFiles(files) {
    ...
    for (const file of files) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        28tools.showStatus('statusArea', 'error', `${file.name} is too large (max 10MB). Skipped.`);
        continue;
      }
    ...
    if (added > 0) {
      ...
      renderList();
      28tools.showStatus('statusArea', 'success', `${added} image(s) added. Total: ${imageFiles.length}. Reorder if needed, then Convert.`);
    }
  }

  async function convertToPDF() {
    if (!imageFiles.length) { 28tools.showStatus('statusArea', 'error', 'Add at least one image.'); return; }
    if (typeof PDFLib === 'undefined') { 28tools.showStatus('statusArea', 'error', 'PDF library not loaded yet. Please wait.'); return; }

    28tools.showStatus('statusArea', 'info', 'Converting…');
    ...
    try {
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        28tools.showStatus('statusArea', 'info', `Processing image ${i + 1}/${imageFiles.length}…`);
      ...
      28tools.showStatus('statusArea', 'success', `PDF created with ${imageFiles.length} pages! Size: ${28tools.formatBytes(resultBytes.byteLength)}`);
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBytes) {
      ...
      28tools.downloadBlob(blob, 'images-converted.pdf');
    }
  });

  document.getElementById('clearAllBtn').addEventListener('click', () => {
    imageFiles = []; renderList(); workspace.style.display = 'none';
    28tools.showStatus('statusArea', 'info', 'All images cleared.');
  });
