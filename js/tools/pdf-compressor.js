(function(){
  ...
  function loadFile(file){ currentFile=file; previewBody.innerHTML=`<div class="status-msg status-msg--success">✅ PDF ready: ${28tools.formatBytes(file.size)}</div>`; sizeInfo.textContent=28tools.formatBytes(file.size); workspace.style.display=''; }
  ...
  processBtn.addEventListener('click',async()=>{
    ...
    s1.onload=async()=>{
      ...
      s2.onload=async()=>{
        ...
        previewBody.innerHTML=`<div class="status-msg status-msg--success">✅ Compressed!<br>Original: ${28tools.formatBytes(currentFile.size)}<br>Compressed: ${28tools.formatBytes(blob.size)}<br>Saved: ${28tools.formatBytes(Math.max(0,saved))} (${Math.max(0,pct)}%)</div>`;
        sizeInfo.textContent=`${28tools.formatBytes(blob.size)} (${Math.max(0,pct)}% smaller)`;
  ...
  downloadBtn.addEventListener('click',()=>{ if(window._compressedPdf) 28tools.downloadBlob(window._compressedPdf,'compressed.pdf'); });
