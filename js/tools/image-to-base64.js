(function(){
  ...
  function loadFile(file){
    ...
    const img=new Image();
    img.onload=()=>{ previewBody.innerHTML=`<img src="${url}" class="preview-img" alt="Preview">`; sizeInfo.textContent=`${img.naturalWidth}×${img.naturalHeight} | ${ToolsApp.formatBytes(file.size)}`; workspace.style.display=''; };
    img.src=url;
  }
  processBtn.addEventListener('click',()=>{
    ...
    reader.onload=e=>{
      ...
      sizeInfo.textContent=`Base64 size: ${ToolsApp.formatBytes(output.length)} chars`;
  ...
  downloadBtn.addEventListener('click',()=>{
    const text=document.getElementById('b64Text')?.value;
    if(text){ const blob=new Blob([text],{type:'text/plain'}); ToolsApp.downloadBlob(blob,(currentFile?currentFile.name.replace(/\.[^.]+$/,''):'image')+'-base64.txt'); }
  });
