(function(){
  ...
  downloadBtn.addEventListener('click',async()=>{
    ...
    if(imgs.length===1){ToolsApp.downloadBlob(imgs[0].blob,imgs[0].name);return;}
    ...
    s2.onload=async()=>{ const zip=new JSZip(); imgs.forEach(({blob,name})=>zip.file(name,blob)); const zb=await zip.generateAsync({type:'blob'}); ToolsApp.downloadBlob(zb,'pdf-pages.zip'); downloadBtn.disabled=false;downloadBtn.innerHTML='⬇️ Download Images ZIP'; };
