(function(){
  ...
  function renderList(){
    document.getElementById('fileListArea').innerHTML=files.map((f,i)=>`<div class="file-item"><span class="file-item__icon">🖼️</span><span class="file-item__name">${f.name}</span><span class="file-item__size">${28tools.formatBytes(f.size)}</span><button class="file-item__remove" onclick="removeMerge(${i})">✕</button></div>`).join('');
  }
  ...
  downloadBtn.addEventListener('click',()=>{ if(resultBlob){ const ext=fmt==='image/jpeg'?'jpg':'png'; 28tools.downloadBlob(resultBlob,'merged.'+ext); } });
  document.getElementById('downloadBtn').addEventListener('click',()=>{ if(resultBlob){ 28tools.downloadBlob(resultBlob,'merged-image.png'); } });
