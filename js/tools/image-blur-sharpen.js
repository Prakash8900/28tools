(function () {
    ...
    setTimeout(() => {
      try {
        ...
        canvas.toBlob(blob => {
          ...
          ToolsApp.showStatus('statusArea', 'success', 'Effect applied! Click Download to save.');
        }, fmt, 0.92);
      } catch (e) {
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBlob) {
      ...
      const mode = document.getElementById('effectMode').value;
      ToolsApp.downloadBlob(resultBlob, `${base}-${mode}.${ext}`);
    }
  });

  function loadImage(file) {
    if (!file.type.startsWith('image/')) {
      ToolsApp.showStatus('statusArea', 'error', 'Please upload an image file (JPG, PNG, WebP).');
      return;
    }
    ...
    img.onload = () => {
      ...
      resultBlob = null;
      ToolsApp.showStatus('statusArea', 'success', 'Image loaded. Adjust settings and click Apply.');
    };
    img.onerror = () => ToolsApp.showStatus('statusArea', 'error', 'Could not load image.');
    img.src = url;
  }
