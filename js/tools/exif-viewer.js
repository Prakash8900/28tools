(function(){
  ...
  function loadFile(file){
    ...
    const img=new Image();
    img.onload=()=>{ previewBody.innerHTML=`<img src="${url}" class="preview-img" alt="Preview">`; sizeInfo.textContent=`${img.naturalWidth}×${img.naturalHeight} | ${ToolsApp.formatBytes(file.size)}`; workspace.style.display=''; readExif(file); };
    img.src=url;
  }
  ...
  function parseBasicExif(view,type){
    const data={};
    data['File Type']=type;
    data['File Size']=ToolsApp.formatBytes(view.byteLength);
    if(type==='image/jpeg'){
      try{
        let offset=2;
  ...
  document.getElementById('stripBtn').addEventListener('click',()=>{
    ...
    img.onload=()=>{
      ...
      canvas.toBlob(blob=>{
        ...
        sizeInfo.textContent=`Metadata removed | ${ToolsApp.formatBytes(blob.size)}`;
  ...
  downloadBtn.addEventListener('click',()=>{ if(cleanedBlob) ToolsApp.downloadBlob(cleanedBlob,(currentFile?currentFile.name.replace(/\.[^.]+$/,''):'img')+'-clean.png'); });
