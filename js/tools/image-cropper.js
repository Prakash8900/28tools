(function(){
  ...
  cropBtn.addEventListener('click',()=>{
    ...
    out.toBlob(blob=>{
      ...
      cropInfo.textContent=`${sw}×${sh}px | ${ToolsApp.formatBytes(blob.size)}`;
      resultArea.style.display='';
    },'image/png');
  });

  downloadBtn.addEventListener('click',()=>{
    if(resultBlob) ToolsApp.downloadBlob(resultBlob,(origFile?origFile.name.replace(/\.[^.]+$/,''):'image')+'-cropped.png');
  });
