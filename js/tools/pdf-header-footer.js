(function () {
  ...
  async function addHeaderFooter() {
    ...
    if (!headerText && !footerText) {
      28tools.showStatus('statusArea', 'error', 'Please enter at least a header or footer text.'); return;
    }
    ...
    try {
      ...
      28tools.showStatus('statusArea', 'success', `Header/Footer added to all ${total} pages!`);
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBytes) {
      ...
      28tools.downloadBlob(blob, `${base}-with-header-footer.pdf`);
    }
  });
  ...
  function loadFile(file) {
    ...
    reader.onload = async e => {
      ...
      resultBytes = null;
      28tools.showStatus('statusArea', 'success', 'PDF loaded. Enter header/footer text and click Add.');
    };
