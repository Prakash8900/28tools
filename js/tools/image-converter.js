(function(){
  ...
  function renderList(){
    fileListArea.innerHTML=files.map((f,i)=>`<div class="file-item"><span class="file-item__icon">🖼️</span><span class="file-item__name">${f.name}</span><span class="file-item__size">${28tools.formatBytes(f.size)}</span></div>`).join('');
  }
  function previewFile(f){
    const url=URL.createObjectURL(f);
    const img=new Image();
    img.onload=()=>{ previewBody.innerHTML=`<img src="${url}" class="preview-img" alt="Original">`; sizeInfo.textContent=`${img.naturalWidth}×${img.naturalHeight} | ${28tools.formatBytes(f.size)}`; };
    img.src=url;
  }
  ...
  convertBtn.addEventListener('click',async()=>{
    ...
    const img=new Image();
    img.onload=()=>{ previewBody.innerHTML=`<img src="${url}" class="preview-img" alt="Converted">`; sizeInfo.textContent=`Converted: ${img.naturalWidth}×${img.naturalHeight} | ${28tools.formatBytes(resultBlobs[0].blob.size)}`; };
    img.src=url;
    downloadArea.style.display='';
    convertBtn.disabled=false; convertBtn.innerHTML='🔄 Convert Images';
  });
  downloadBtn.addEventListener('click',async()=>{
    if(resultBlobs.length===1){ 28tools.downloadBlob(resultBlobs[0].blob,resultBlobs[0].name); return; }
    downloadBtn.disabled=true; downloadBtn.innerHTML='<span class="spinner"></span> Zipping…';
    const script=document.createElement('script');
    script.src='https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js';
    script.onload=async()=>{ const zip=new JSZip(); resultBlobs.forEach(({blob,name})=>zip.file(name,blob)); const zb=await zip.generateAsync({type:'blob'}); 28tools.downloadBlob(zb,'converted-images.zip'); downloadBtn.disabled=false; downloadBtn.innerHTML='⬇️ Download Converted Image(s)'; };
