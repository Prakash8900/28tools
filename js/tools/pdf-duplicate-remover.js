(function () {
  ...
  async function detectAndRemove() {
    ...
    if (typeof pdfjsLib === 'undefined' && !window.pdfjsLib) {
      ...
      28tools.showStatus('statusArea', 'info', 'Loading PDF rendering engine…');
      await new Promise(resolve => setTimeout(resolve, 500));
    }

    28tools.showStatus('statusArea', 'info', 'Rendering pages for comparison… (may take a moment)');
    ...
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        28tools.showStatus('statusArea', 'info', `Analyzing page ${i}/${pageCount}…`);
      ...
      if (removedPages.length === 0) {
        ...
        28tools.showStatus('statusArea', 'success', 'No duplicates found!');
      ...
      if (!dryRun) {
        ...
        28tools.showStatus('statusArea', 'success', `Removed ${removedPages.length} duplicate page(s). Download ready!`);
      } else {
        28tools.showStatus('statusArea', 'success', `Found ${removedPages.length} duplicate(s). Uncheck "Preview only" and click again to remove them.`);
      }
    } catch (e) {
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBytes) {
      ...
      28tools.downloadBlob(blob, `${base}-deduped.pdf`);
    }
  });
  ...
  function loadFile(file) {
    ...
    reader.onload = e => {
      ...
      resultBytes = null;
      28tools.showStatus('statusArea', 'success', 'PDF loaded. Click Detect & Remove Duplicates.');
    };
