(function () {
  ...
  async function addPageNumbers() {
    if (!pdfBytes) return;
    if (typeof PDFLib === 'undefined') { ToolsApp.showStatus('statusArea', 'error', 'PDF library not loaded yet. Please wait a moment.'); return; }

    ToolsApp.showStatus('statusArea', 'info', 'Adding page numbers…');
    ...
    try {
      ...
      ToolsApp.showStatus('statusArea', 'success', `Page numbers added to all ${total} pages!`);
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBytes) {
      ...
      ToolsApp.downloadBlob(blob, `${base}-numbered.pdf`);
    }
  });
  ...
  function loadFile(file) {
    ...
    if (file.size > 50 * 1024 * 1024) {
      ToolsApp.showStatus('statusArea', 'error', 'File too large. Max 50MB.'); return;
    }
    origFile = file;
    ...
    reader.onload = async e => {
      ...
      if (typeof PDFLib !== 'undefined') {
        try {
          ...
          ToolsApp.showStatus('statusArea', 'success', `PDF loaded: ${pageCount} pages. Configure settings and click Add Page Numbers.`);
        } catch (err) {
          previewBody.innerHTML = `<p>${file.name} — ${ToolsApp.formatBytes(file.size)}</p>`;
          ToolsApp.showStatus('statusArea', 'info', 'PDF loaded. Configure settings and click Add Page Numbers.');
        }
      } else {
        previewBody.innerHTML = `<p>${file.name} — ${ToolsApp.formatBytes(file.size)}</p>`;
        ToolsApp.showStatus('statusArea', 'info', 'PDF loaded. Configure settings and click Add Page Numbers.');
