(function () {
  ...
  async function convert() {
    if (!pdfBytes) return;
    if (typeof PDFLib === 'undefined') { 28tools.showStatus('statusArea', 'error', 'PDF library not loaded. Please wait.'); return; }
    if (!window.pdfjsLib) { 28tools.showStatus('statusArea', 'error', 'PDF.js not loaded. Please refresh.'); return; }

    28tools.showStatus('statusArea', 'info', 'Loading PDF…');
    ...
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        28tools.showStatus('statusArea', 'info', `Converting page ${i}/${pageCount}…`);
      ...
      28tools.showStatus('statusArea', 'success', `Converted ${pageCount} pages to grayscale! Size: ${28tools.formatBytes(resultBytes.byteLength)}`);
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBytes) {
      ...
      28tools.downloadBlob(blob, `${base}-grayscale.pdf`);
    }
  });
  ...
  function loadFile(file) {
    ...
    if (file.size > 30 * 1024 * 1024) { 28tools.showStatus('statusArea', 'error', 'File too large. Max 30MB.'); return; }
    origFile = file;
    const reader = new FileReader();
    reader.onload = e => {
      pdfBytes = new Uint8Array(e.target.result);
      previewBody.innerHTML = `<p style="color:var(--clr-text-muted)">${file.name} — ${28tools.formatBytes(file.size)}</p>`;
      workspace.style.display = '';
      downloadArea.style.display = 'none';
      resultBytes = null;
      28tools.showStatus('statusArea', 'success', 'PDF loaded. Select quality and click Convert.');
    };
