(function () {
  ...
  async function analyzeFile(file) {
    ...
    ToolsApp.showStatus('statusArea', 'info', 'Analyzing PDF…');
    ...
    reader.onload = async e => {
      try {
        ...
        if (typeof PDFLib === 'undefined') {
          ToolsApp.showStatus('statusArea', 'error', 'PDF library not loaded yet. Please wait and try again.');
          return;
        }
        ...
        infoContent.innerHTML = `
          ...
          <table class="info-table">
            ...
            <tr><td>File Size</td><td>${ToolsApp.formatBytes(file.size)} (${file.size.toLocaleString()} bytes)</td></tr>
        ...
        ToolsApp.showStatus('statusArea', 'success', `Analysis complete! ${pageCount} page(s) found.`);
      } catch (err) {
        ToolsApp.showStatus('statusArea', 'error', 'Could not read PDF: ' + err.message);
      }
    };
