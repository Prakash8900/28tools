(function () {
  ...
  function generatePhoto() {
    if (!img) return;
    28tools.showStatus('statusArea', 'info', 'Generating…');
    setTimeout(() => {
      try {
        ...
        canvas.toBlob(blob => {
          ...
          sizeInfo.textContent = `${w}×${h}px | ${28tools.formatBytes(blob.size)}`;
          downloadArea.style.display = '';
          28tools.showStatus('statusArea', 'success', 'Passport photo generated! Click Download to save.');
        }, fmt, 0.95);
      } catch (e) {
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBlob) {
      ...
      const sizeKey = document.getElementById('sizeSelect').value;
      28tools.downloadBlob(resultBlob, `passport-photo-${sizeKey}.${ext}`);
    }
  });
  ...
  function loadImage(file) {
    ...
    img.onload = () => {
      ...
      resultBlob = null;
      28tools.showStatus('statusArea', 'success', 'Photo loaded. Select size and click Generate.');
    };
    img.src = url;
  }
