(function(){
  ...
  function applyTransform(){
    ...
    canvas.toBlob(blob=>{
      ...
      sizeInfo.textContent=`${nw}×${nh} | ${28tools.formatBytes(blob.size)}`;
  ...
  downloadBtn.addEventListener('click',()=>{
    if(resultBlob){ const ext=resultBlob.type.split('/')[1].replace('jpeg','jpg'); 28tools.downloadBlob(resultBlob,(origFile?origFile.name.replace(/\.[^.]+$/,''):'img')+'-rotated.'+ext); }
  });
