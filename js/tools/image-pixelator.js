(function () {
  ...
  function pixelate() {
    if (!img) return;
    ToolsApp.showStatus('statusArea', 'info', 'Pixelating…');
    setTimeout(() => {
      try {
        ...
        canvas.toBlob(blob => {
          ...
          sizeInfo.textContent = `${w}×${h}px | Block: ${blockSize}px | ${ToolsApp.formatBytes(blob.size)}`;
          downloadArea.style.display = '';
          ToolsApp.showStatus('statusArea', 'success', 'Pixelation applied! Click Download to save.');
        }, fmt, 0.92);
      } catch (e) {
  ...
  downloadBtn.addEventListener('click', () => {
    if (resultBlob) {
      ...
      ToolsApp.downloadBlob(resultBlob, `${base}-pixelated.${ext}`);
    }
  });
  ...
  function loadImage(file) {
    ...
    img.onload = () => {
      ...
      resultBlob = null;
      ToolsApp.showStatus('statusArea', 'success', 'Image loaded. Set block size and click Apply.');
    };
    img.src = url;
  }
